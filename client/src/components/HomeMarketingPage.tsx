...
const VIDEO_SLOTS = [
  { id: 'video-1', title: 'Feature Video Slot', embedUrl: DEFAULT_HERO_VIDEO },
  { id: 'video-2', title: 'Hype Reel Slot', embedUrl: '' },
];
      ...
      <main>
        ...
        <section className="container mx-auto px-4 py-14">
          <div className="max-w-3xl mb-7">
            ...
            <h2 className="text-3xl font-black mt-3">Video Section Ready For Your Links</h2>
            <p className="text-gray-300 mt-3">Top hero video is loaded from your previous page. Keep adding your next video links into the second slot.</p>
          </div>
            ...
            {VIDEO_SLOTS.map((slot) => (
              <div key={slot.id} className="rounded-xl border border-gray-800 bg-gray-950 p-4">
                {slot.embedUrl ? (
                  ...
                  <div className="w-full aspect-video rounded-lg border border-dashed border-gray-700 flex items-center justify-center text-gray-500">
                    {slot.title} (Video URL placeholder)
                  </div>
                )}
              </div>
            ))}
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {BOTTOM_IMAGE_SLOTS.map((slot) => (
              <div key={slot.id} className="rounded-xl border border-gray-800 bg-gray-950 overflow-hidden">
                ...
                <div className="p-3 text-sm text-gray-300">{slot.title}</div>
              </div>
            ))}
          </div>
        </section>
